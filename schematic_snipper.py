...
class SchematicSnipper:
    def __init__(self, root):
        self.root = root
        self.root.title("Schematic Snipper - Tight Crops")
    ...
    def setup_ui(self):
        ...
        tk.Label(self.toolbar, text="Snip Width (px):", fg="white", bg="#2c3e50").pack(side=tk.LEFT, padx=5)
        ...
        self.pdf_canvas = tk.Canvas(self.pdf_container, bg="#333", xscrollcommand=self.h_scroll.set, yscrollcommand=self.v_scroll.set)
    ...
    def render_all_pages(self):
        ...
        self.status_label.config(text=f"Loaded: {len(self.doc)} Pages")
        self.progress.pack_forget()
    ...
    def process_snip(self, x1, y1, x2, y2):
        ...
        if clip.width < 5 or clip.height < 5: return
    ...
    def add_to_sidebar(self, img, page_info):
        ...
        # --- The Fix for White Space ---
        # Calculate height to maintain aspect ratio without adding padding
        ...
        frame = tk.Frame(self.snip_list_frame, bd=2, relief="ridge", bg="white")
        frame.pack(pady=5, padx=10, fill=tk.X)
        
        lbl = tk.Label(frame, image=tk_img, bg="white")
        lbl.image = tk_img # reference
        lbl.pack()
        
        tk.Label(frame, text=f"Page {page_info}", bg="#f1f1f1", font=("Arial", 8, "italic")).pack(fill=tk.X)
    ...
    def save_workspace(self):
        folder = filedialog.asksaveasfilename(title="Save Workspace Folder")
        ...
        messagebox.showinfo("Saved", "Workspace folder created successfully.")
